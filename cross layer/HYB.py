...
LOG_FILE = './results/log_sim_hyb'
beta = 0.6  # parameter that controls the aggressiveness of algorithm
...
def main():
    ...
    while True:  # serve video forever
        ...
        candidates = []
        for i in VIDEO_BIT_RATE:
            if i*M_IN_K/8*2/future_bandwidth < beta*buffer_size:
                candidates.append(i)

        bit_rate = VIDEO_BIT_RATE.index(max(candidates))
        if retrans:
            bit_rate = 0
